...
Config_t parse_args(int argc, char *argv[])
{
	...
	desc.add_options()
		...
		("scene,s",
			po::value<std::string>()->default_value(""),
			"Path to constellation configuration file"
		)
		("strat,",
			po::value<std::string>()->default_value(""),
        ...
        ("type,t",
             ...
             "Select type of input image (0：lightImage,1：focuesImage):\n"
         )
        ;
	...
	if(		vm["pimages"].as<std::string>() == "" or vm["pparams"].as<std::string>() == ""
		or 	vm["pcamera"].as<std::string>() == "" or vm["scene"].as<std::string>() == ""  
	)
	{
		/* print usage */
    ...
    // add by xyy
    config.type			    = vm["type"].as<std::uint16_t>();
